...
- **F1 Composition + waste indictment** *(headline)* — decompose assembled prompt by source (system / each tool schema / each MCP / RAG / history / memory / skills); % + abs tokens; indict: unused-loaded-tools, verbatim-duplicated blocks, repeated RAG chunks across turns, never-pruned history, preamble re-payment count, cache-prefix-break (cacheable-prefix instability across same-provider+model turns — D-010), request-replayed (byte-identical full-body retry/re-send + re-billed token weight + buffered status — C6/D-013), component-drift (a same-named system/tool-schema component silently mutating mid-session — the opposite of preamble-repay — C2/D-011), param-drift (a tracked sampling/decoding request field — temperature/top_p/top_k/max_tokens/stop/penalties/seed/response_format/tool_choice — changing value between two same-provider+model turns; a reported determinism-surface fact, never a non-determinism *attribution* — that is `agentlock`'s; C4/D-014), non-text-payload (multimodal/file content — base64 data URIs, image/image_url/input_audio/file/document blocks, both wire shapes — attributed as a DISTINCT component + indictment by EXACT wire byte weight + block counts + % of the assembled body, instead of silently inflating history; base64 NOT decoded, the per-image token estimate OMITTED ENTIRELY to stay strictly pure, `wasted_tokens` hard-0; C5/D-015). Plus the **context-window headroom & growth-rate slope** measurement (C3 — D-012): the focus step’s `prompt_tokens` as a pure-arithmetic fraction of the model’s window (a static offline registry keyed by the wire model id, §8 seam, a labeled approximation like the ±N% tokenizer) and the measured tokens/turn slope across same-(provider,model) turns; the headline carries the fraction + slope only — the “turns remaining at the observed mean rate” extrapolation is a **neutral arithmetic projection, `--deep`-only**, never asserted as fate (unknown model id ⇒ no window claim; < 2 same-namespace turns ⇒ no slope). Plus **header-drift** (C7 — D-016, LIVE-ONLY): tracked request-header facts (anthropic-version/-beta, openai-beta, content-encoding, accept-encoding) changing across same-(provider,model) turns — pure value (in)equality, `wasted_tokens` hard-0, auth already REDACTED so no secret; `store.rs` does not persist headers (D-009) so `ctx open` of a saved session honestly stays SILENT — the full post-hoc C7 is the recorded DEFERRED extension. **Form:** `/context`-style breakdown table/bar; `--deep` per-source detail. Subsumes the "loaded skillset" half of hypothesis 1.
